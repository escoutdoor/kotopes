...
func NewTracingConfig() (TracingConfig, error) {
	...
	if serviceName == "" {
		return nil, fmt.Errorf("service name is not defined")
	}

	collectorHost := os.Getenv(tracingCollectorHostEnvName)
	if collectorHost == "" {
		return nil, fmt.Errorf("collector host is not defined")
	}

	collectorPort := os.Getenv(tracingCollectorPortEnvName)
	if collectorPort == "" {
		return nil, fmt.Errorf("collector port is not defined")
	}
